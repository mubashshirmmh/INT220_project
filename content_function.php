<?php
    ...
    function dispsubcategories($parent_id){
        ...
        while($row=mysqli_fetch_assoc($select)){
            echo "<tr><td class='category_title'><a href='/project_php/topics.php?cid=".$row['cat_id']."&scid=".$row['subcat_id']."'>
    ...
    function disptopics($cid,$scid){
        ...
        if(mysqli_num_rows($select)!=0){
            ...
            while($row=mysqli_fetch_assoc($select)){
                echo "<tr><td><a href='/project_php/readtopic.php?cid=".$cid.="&scid=".$scid."&tid=".$row['topic_id']."'>
                     ".$row['title']."</a></td><td>".$row['author']."</td><td>".$row['date_posted']."</td><td>".$row['views']."</td>
                     <td>".$row['replies']."</td></tr>";
            }
            echo "</table>";
        } else {
            echo "<p>This category has no topics yet!! <a href='/project_php/newtopic.php?cid=".$cid."&scid=".$scid."'>
	...
	function replylink($cid, $scid, $tid) {
		echo "<p><a href='/project_php/replyto.php?cid=".$cid."&scid=".$scid."&tid=".$tid."'>Reply to this post</a></p>";
	}
	
	function replytopost($cid, $scid, $tid) {
		echo "<div class='content'><form action='/project_php/addreply.php?cid=".$cid."&scid=".$scid."&tid=".$tid."' method='POST'>
			  <p>Comment: </p>
